...
<div class="w-full">
	<div class="mr-2 flex flex-row justify-end">
		<button on:click={hideMenu} class="p-2 text-xs text-slate-400">{hide ? 'Show' : 'Hide'}</button>
	</div>
	<div class="flex-grow p-4 text-xs font-mono">
		...
		{#if ready && !hide}
			<nav
				transition:fadein={{ delay: activated ? 0 : 3000 }}
